...
export class ConfigurationValidator {
   ...
   */
  private static validateHederaPrivateKey = (
    value: string,
    ...
    try {
      ...
      return helpers.error('any.invalid', {
        message:
          'Invalid Hedera private key format. Supported formats: 64-character hex (raw), DER/PKCS#8 hex, or Hedera SDK string format',
      });
    }
  };
  ...
  private static readonly configSchema = Joi.object({
    app: Joi.object({
      ...
      defaultRewardClaimDuration: Joi.number()
        .integer()
        .min(1)
        .max(365)
        .required(),
      defaultCampaignDuration: Joi.number()
        .integer()
        .min(1)
        .max(365)
        .required(),
    ...
    network: Joi.object({
      network: Joi.string()
        .valid(...Object.values(HederaNetwork))
        .required(),
      privateKey: Joi.string()
        .custom(ConfigurationValidator.validateHederaPrivateKey)
        .required(),
   ...
   */
  static validatePartialConfig(config: Partial<AppConfig>): {
    isValid: boolean;
    errors: string[];
  } {
    try {
   ...
   */
  static validateSecurity(config: AppConfig): {
    isSecure: boolean;
    warnings: string[];
  } {
    ...
    if (config.encryptions.jwtSecretForAccessToken.length < 64) {
      warnings.push(
        'JWT access token secret should be at least 64 characters for enhanced security'
      );
    }

    if (config.encryptions.jwtSecretForRefreshToken.length < 64) {
      warnings.push(
        'JWT refresh token secret should be at least 64 characters for enhanced security'
      );
    }
    ...
    if (config.app.environment === Environment.PRODUCTION) {
      if (
        config.app.appURL.includes('localhost') ||
        config.app.appURL.includes('127.0.0.1')
      ) {
    ...
    if (config.cache.configCacheTtlMs < 300000) {
      // 5 minutes
      warnings.push(
        'Configuration cache TTL is very low, consider increasing for better performance'
      );
    }
   ...
   */
  static validateEnvironment(config: AppConfig): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    switch (config.app.environment) {
      case Environment.PRODUCTION:
        ...
        if (!config.featureFlags.rateLimiting) {
          errors.push(
            'Rate limiting must be enabled in production environment'
          );
        }
        break;
