...
const calculateRespectivewidth = (height: number): number => Math.floor((height * 300) / 352);
const calculateRespectivheight = (width: number): number => Math.floor((width * 352) / 300);
const HashpackIcon = ({width, height}: Props) => {
  return (
    <svg   width={height ? calculateRespectivewidth(height) : width ?? ""}
    height={width ? calculateRespectivheight(width) : height ?? ""} fill="none" viewBox="0 0 300 352" xmlns="http://www.w3.org/2000/svg">
      <path
