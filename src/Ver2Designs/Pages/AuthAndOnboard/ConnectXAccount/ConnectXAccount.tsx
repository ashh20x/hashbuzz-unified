...
const ConnectXAccount = () => {
  ...
  const [getTwitterPersonalHandle, { isLoading: isLoadingTwitterHandle }] =
    ...
    <Box sx={styles.connectXAccountStyles}>
      <Box id='content-container'>
        ...
        <Box sx={styles.sectionTopContent}>
          ...
          <p>
            Each Hedera account can be linked to one 𝕏 account. Once linked,
        ...
        <Box sx={styles.sectionBottomContent}>
          ...
          <Alert
            sx={styles.alertInfoContainer}
            variant='outlined'
            severity='info'
          >
            We will never access private DMs or share your data without consent.
          </Alert>
        </Box>
      </Box>
      <Stack
        className='button-container-connect-x-account'
        sx={{ p: 2 }}
        direction='row'
        justifyContent='flex-end'
      >
        <PrimaryButtonV2
          loading={isLoadingTwitterHandle}
