...
const HeroSection = () => {
  ...
  return (
    <Box
      ...
      sx={styles.heroSectionContainer}
    >
      <Box
        component='div'
        id='hero-section-content'
        sx={styles.heroSectionContentArea}
      >
        <Box id='hero-section-content-area' sx={styles.heroSectionContent}>
          ...
          <p>
            Connect your wallet, link your 𝕏 account, and start rewarding real
            interactions using crypto tokens — all in a few clicks.
          </p>
          <Box id='hero-section-btns' className='heroSectionBtns'>
            <Button
              onClick={handleOpenVideoPlayer}
              sx={styles.howItWorksButton}
              disableElevation
              size='medium'
              variant='contained'
              startIcon={<PlayCircle fontSize='inherit' />}
            >
              How it works
            </Button>
            <Button
