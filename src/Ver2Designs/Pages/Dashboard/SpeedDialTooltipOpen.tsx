...
const actions = [
  { icon: <img src={ℏicon} alt={"ℏ"} style={{ height: "24px", width: "auto", marginRight: 10, display: "inline-block" }} />, name: "Top" },
      ...
      >
        {actions.map((action) => {
          ...
          <SpeedDialAction
            ...
            tooltipOpen
            //@ts-ignore
            onClick={(event) => handleClick(action.name)}
          />
        )})}
      </SpeedDial>
      <QRCodeDialog open={qrCodeOpen} onclose={() => {
        setQrCodeOpen(false);
        // handleClose()
      }} />
    </React.Fragment>
