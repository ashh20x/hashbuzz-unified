...
const BalanceCard = ({ entityBal, entityIcon }: { entityBal: string | number; entityIcon: React.ReactNode }) => {
...
const Balances = () => {
  ...
  const [balanceList, setBalanceList] = useState<{ operation: BalOperation }>({ operation: "topup" });
  const topUpButtonsListRef = useRef<HTMLDivElement>(null);
  const [triggerGetBalancesForAccountId] = useLazyGetBalancesForAccountIdQuery();
  ...
  const handleMenuItemClick = async (
    ...
    if (balanceList.operation === "topup") {
      ...
      if (accountId) {
        const accountBalReq = await triggerGetBalancesForAccountId(accountId).unwrap();
        const accountBal = accountBalReq.balances.find(b => b.account === accountId);
  ...
  const handleTopupOrReimClick = (operation: BalOperation, event?: React.MouseEvent) => {
    unstable_batchedUpdates(() => {
      ...
      setEntityEl(event ? (event.target as HTMLElement) : topUpButtonsListRef.current);
    });
  };
              ...
              >
                ...
                {balances && balances.length > 0 && (
                  <ButtonGroup
                    ...
                    sx={{ ".MuiButton-startIcon": { m: 0 }, justifyContent: "center" }}
