...
const campaignSlice = createSlice({
  ...
  reducers: {
    // Form data updates
    updateFormField: (state, action: PayloadAction<{ field: keyof CampaignFormData; value: string }>) => {
    ...
    addMediaFile: (state, action: PayloadAction<MediaFile>) => {
      ...
      } else if (state.media.length < 4 && !mediaFile.type.includes('gif') && !state.gifSelected) {
        state.displayMedia.push(mediaFile.url)
      } else {
    ...
    // Token management
    setAllTokens: (state, action: PayloadAction<Array<{ value: string; token_symbol: string; tokenBalance: number }>>) => {
    ...
    // Budget validation
    validateBudget: (state, action: PayloadAction<{ budget: string; userBalance: number; decimals?: number }>) => {
      const { budget, userBalance, decimals = 8 } = action.payload
      const budgetValue = parseFloat(budget)
      
      if (state.formData.type === 'HBAR') {
        const budgetInTinyHbar = Math.round(budgetValue * Math.pow(10, decimals))
        ...
        const currentToken = state.allTokens.find(token => token.value === state.selectedToken)
    ...
    addEmoji: (state, action: PayloadAction<string>) => {
      ...
      if (270 - currentLength >= 2) {
        ...
        // Update button tags
        const hashtags = state.formData.tweet_text.split(' ').filter(item => item.startsWith('#'))
        state.buttonTags = hashtags
      }
    },
    ...
    // Error messages
    setErrorMessage: (state, action: PayloadAction<{ field: 'name' | 'text' | 'budget'; message: string }>) => {
