...
import { AppState, EntityBalances, ContractInfo, CurrentUser, AuthCred } from '../types';
...
const miscellaneousStoreSlice = createSlice({
    ...
    initialState,
    reducers: {
        setPing: (state: AppState, action: PayloadAction<{ status: boolean; hedera_wallet_id: string }>) => {
            state.ping = action.payload;
            state.checkRefresh = true;
        },
        updateState: (state: AppState, action: PayloadAction<Partial<AppState>>) => {
        ...
        addToast: (state: AppState, action: PayloadAction<{ type: 'success' | 'error'; message: string }>) => {
