...
export const handleGetAllUser = async (
  ...
  try {
    ...
    const userData = await Promise.all(
      users.map(async (u) =>
        JSONBigInt.parse(JSONBigInt.stringify(await sensitizeUserData(u))) as Awaited<ReturnType<typeof sensitizeUserData>>
      )
    );
...
export const handleCurrentUser = async (
  ...
  if (req?.accountAddress) {
    ...
    const currentUser = await userService.getUserByAccountAddress(
      req.accountAddress
    );
    ...
    if (currentUser) {
      ...
      return res
        .status(OK)
        .json({
          ...JSONBigInt.parse(JSONBigInt.stringify(senetizedUser)),
          config: {
            contractAddress,
            collecterAddress,
            campaignDuration,
            campaignRewardDuration,
          },
        });
...
export const handleUpdateConcent = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const handleGetUserBalances = (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  if (contractBal) {
    (async () => {
      ...
      if (req.currentUser?.id && balances?.balances)
      ...
      logger.info(
        `Contract balance for the ${address} is::::- ${balances?.balances ?? 0}`
      );
    ...
    return res
      .status(OK)
      .json({ available_budget: req.currentUser?.available_budget });
  }
};

export const handleTokenBalReq = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  const balanceData = tokenList.map((token) => {
    const balance_record = userBalancesForTokens.find(
      (b) => b.token_id === token.id
    );
    return formatTokenBalancesObject(token, balance_record);
  });
  return res
    .status(OK)
    .json(JSONBigInt.parse(JSONBigInt.stringify(balanceData)));
};
...
export const handleTokenContractBal = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    if (req.currentUser?.hedera_wallet_id) {
      const balance = await queryFungibleBalanceOfCampaigner(
        req.currentUser.hedera_wallet_id,
        tokenId
      );
...
export const syncBal = async (
  ...
  try {
    ...
    if (!tokendata)
      return res
        .status(BAD_REQUEST)
        .json({ error: true, messages: 'Unsupoorted token' });
