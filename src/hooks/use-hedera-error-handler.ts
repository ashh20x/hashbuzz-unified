...
const HEDERA_ERROR_MESSAGES: Record<string, string> = {
  INSUFFICIENT_PAYER_BALANCE:
    'Your account does not have enough HBAR to pay for this transaction.',
  ...
  INSUFFICIENT_ACCOUNT_BALANCE:
    'Account balance too low to complete this transaction.',
...
const COMMON_ERROR_PATTERNS: Array<{ pattern: RegExp; message: string }> = [
  {
    ...
    message:
      'Invalid transaction data received. Please try again or contact support.',
  },
  {
  ...
  const parseHederaError = (error: any) => {
    ...
    if (error?.txError || error?.queryError) {
      if (error.txError?.message === 'USER_REJECT') {
        return {
          status: 'REJECTED',
          message: 'Transaction signing was cancelled by the user.',
        };
      }
    ...
    if (error instanceof StatusError) {
      const statusMessage =
        HEDERA_ERROR_MESSAGES[error.status.toString()] ||
    ...
    if (error?.data?.message) {
      return {
        status: 'FAILED',
        message: getUserFriendlyMessage(error.data.message),
      };
    }
