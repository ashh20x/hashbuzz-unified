...
import { logDebug, logInfo } from "../hooks/session-manager/utils";
...
export const useSmartContractServices = () => {
  ...
  const topUpAccount = async (entity: CreateTransactionEntity) => {
    try {
      ...
      logDebug("Transaction bytes response received", JSON.stringify(transactionBytesResponse));
      ...
      if (transactionBytesResponse?.data) {
        ...
        throw new Error("Invalid response: No transaction data received from server");
      }
      ...
      logDebug("Processing transaction bytes", `Type: ${typeof transactionBytes}, Length: ${transactionBytes.length || "unknown"}`);
      ...
      try {
        if (typeof transactionBytes === "string") {
          ...
          if ((transactionBytes as string).startsWith("0x")) {
            // Hex string
            const bytes = new Uint8Array(Buffer.from((transactionBytes as string).slice(2), "hex"));
            transaction = Transaction.fromBytes(bytes);
          } else {
            // Assume base64
            const bytes = new Uint8Array(Buffer.from(transactionBytes as string, "base64"));
            transaction = Transaction.fromBytes(bytes);
          }
        ...
        logDebug("Failed to parse transaction bytes", `Error: ${parseError.message}`);
        throw new Error(`Invalid transaction format: ${parseError.message}`);
      }

      logDebug("Transaction parsed successfully", `Transaction ID: ${transaction.transactionId}`);

      // Step 2: Sign and execute the transaction
      const signedTx = await transaction.executeWithSigner(signer as any);
      logDebug("Transaction executed successfully", `useSmartContractServices.topUpAccount`);
      ...
      logDebug("Transaction failed", `Error: ${err.message || 'Unknown error'}`);
