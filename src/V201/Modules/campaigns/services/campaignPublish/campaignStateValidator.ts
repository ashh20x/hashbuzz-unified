...
export class CampaignPublishStateValidator {
  ...
  async analyzeCampaignState(
    ...
    try {
      ...
      const stateInfo = this.determineCurrentState(campaign);

      logger.info(`Campaign ${campaignId} state analysis: ${stateInfo.currentState}`);

      return stateInfo;

    } catch (error) {
      logger.err(`Error analyzing campaign state: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  private determineCurrentState(
    ...
    if (!approve) {
      return {
        ...
        errorMessage: 'Campaign requires admin approval before publishing',
        campaign
      };
    }

    // Campaign has not been published at all
    if (card_status === campaignstatus.ApprovalPending || card_status === campaignstatus.CampaignApproved) {
      ...
      return {
        currentState: CampaignPublishState.ERROR_STATE,
        canRetry: false,
        nextAction: 'Contact support - campaign in inconsistent state',
        errorMessage: 'Campaign data is in an inconsistent state',
        campaign
      };
    }
    ...
    return {
      currentState: CampaignPublishState.ERROR_STATE,
      canRetry: false,
      nextAction: 'Check campaign status and contact support if needed',
      errorMessage: `Campaign is in ${card_status} status`,
      campaign
    };
  }
  ...
  async validateCampaignForPublish(
    ...
    try {
      ...
      const canProceed = stateInfo.canRetry || stateInfo.currentState === CampaignPublishState.FULLY_PUBLISHED;

      let validationMessage = '';
      if (!canProceed) {
        validationMessage = stateInfo.errorMessage || 'Campaign cannot be published in current state';
      } else if (stateInfo.currentState === CampaignPublishState.FULLY_PUBLISHED) {
      ...
      logger.err(`Validation error: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
