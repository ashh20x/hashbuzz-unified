...
export class V201CampaignExpiryService {
  ...
  async expireCampaign(campaign: campaign_twittercard): Promise<{
    ...
    try {
      ...
      logger.info(`[V201] Starting campaign expiry for ID: ${campaign.id}, Name: ${campaign.name || ''}`);
      ...
      if (campaign.card_status !== campaignstatus.RewardDistributionInProgress) {
        throw new Error(`Campaign ${campaign.id} is not in RewardDistributionInProgress status`);
      }
      ...
      logger.info(`[V201] Processing final reward distribution for campaign ${campaign.id}`);
      ...
      logger.info(`[V201] Campaign expiry completed successfully for ID: ${campaign.id}`);
      ...
      const errorMsg = error instanceof Error ? error.message : 'Unknown error during campaign expiry';
      logger.err(`[V201] Campaign expiry failed for ID: ${campaign.id}: ${errorMsg}`);
   ...
   */
  private async handleHBARExpiry(campaign: campaign_twittercard, owner: user_user): Promise<void> {
    try {
      ...
      try {
        ...
        logger.info(`[V201] HBAR Smart Contract transaction successful for campaign ${campaign.id}`);
      } catch (err) {
        throw new Error(`Failed to perform HBAR Smart Contract transaction: ${err instanceof Error ? err.message : 'Unknown error'}`);
      }
      ...
      if (balances > 0) {
        try {
          ...
          logger.info(`[V201] Updated user balance for campaign owner ${owner.id}, amount: ${balances}`);
        } catch (err) {
          throw new Error(`Failed to update user balance: ${err instanceof Error ? err.message : 'Unknown error'}`);
        }
      }
      ...
      logger.info(`[V201] HBAR campaign expiry completed for campaign ${campaign.id}`);
    } catch (error) {
      logger.err(`[V201] Error during HBAR expiry for campaign ${campaign.id}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      throw error;
    }
  }
   ...
   */
  private async handleFungibleExpiry(campaign: campaign_twittercard, owner: user_user): Promise<void> {
    try {
      logger.info(`[V201] Processing FUNGIBLE expiry for campaign ${campaign.id}`);
      ...
      try {
        ...
        logger.info(`[V201] FUNGIBLE Smart Contract transaction successful for campaign ${campaign.id}`);
      } catch (err) {
        throw new Error(`Failed to perform FUNGIBLE Smart Contract transaction: ${err instanceof Error ? err.message : 'Unknown error'}`);
      }

      // Step 2: Query balance from Smart Contract
      if (campaign.fungible_token_id) {
        try {
          campaignerBalances = Number(await queryFungibleBalanceOfCampaigner(owner.hedera_wallet_id, campaign.fungible_token_id));
          logger.info(`[V201] Queried balance from Smart Contract for campaign owner ${owner.id}, balance: ${campaignerBalances}`);
        } catch (err) {
          throw new Error(`Failed to query balance from Smart Contract: ${err instanceof Error ? err.message : 'Unknown error'}`);
        }
      }
      ...
      logger.info(`[V201] FUNGIBLE campaign expiry completed for campaign ${campaign.id}`);
    } catch (error) {
      logger.err(`[V201] Error during FUNGIBLE expiry for campaign ${campaign.id}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      throw error;
    }
  }
  ...
  private async publishExpiryAnnouncementTweet(
    ...
    try {
      ...
      logger.err(`[V201] Failed to publish expiry announcement tweet for campaign ${campaign.id}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      throw error;
    }
  }
   ...
   */
  private async generateExpiryAnnouncementText(campaign: campaign_twittercard, type: 'HBAR' | 'FUNGIBLE'): Promise<string> {
  ...
  private async updateCampaignToExpired(campaign: campaign_twittercard): Promise<void> {
    try {
      ...
      logger.info(`[V201] Updated campaign ${campaign.id} status to RewardsDistributed`);
    } catch (error) {
      logger.err(`[V201] Failed to update campaign status for ${campaign.id}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      throw error;
    }
  }
   ...
   */
  private async handleExpiryError(campaign: campaign_twittercard, errorMsg: string): Promise<void> {
    try {
      // Ensure prisma is initialized
      const prisma = await this.initializePrisma();
      ...
      logger.err(`[V201] Updated campaign ${campaign.id} status to InternalError due to: ${errorMsg}`);
    } catch (error) {
      logger.err(`[V201] Failed to handle expiry error for campaign ${campaign.id}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async findCampaignsNeedingExpiry(): Promise<campaign_twittercard[]> {
    try {
      ...
      logger.err(`[V201] Error finding campaigns needing expiry: ${error instanceof Error ? error.message : 'Unknown error'}`);
      return [];
    }
  }
}
