...
contract HashbuzzV2 is HederaTokenService, KeyHelper, ExpiryHelper, RandomNumberGenerate {
  ...
  mapping(address => mapping(address => mapping(uint256 => uint256))) public tokenCampaignBalances;
  ...
  event InteractorPaid(address interactor, address campaigner, uint256 amount);
  event InteractorPaidDeferred(address campaignAddress, uint256 amount);
  event NewCampaignIsAdded(address campaignAdddress, uint256 amount, uint32 campaignType);
  ...
  function transferTokenToContract(address tokenId, address from, int64 tokenAmount) public {
    (int64 responses, int32 tokenType) = HederaTokenService.getTokenType(tokenId);

    if (responses == HederaResponseCodes.SUCCESS) {
      if (tokenType == 0) {
        int response = HederaTokenService.transferToken(tokenId, from, address(this), tokenAmount);

        if (response == HederaResponseCodes.SUCCESS) {
          tokenBalances[from][tokenId][FUNGIBLE] += uint64(int64(tokenAmount));
        } else {
          revert();
        }
      }
  ...
  function tokenAssociate(address sender, address tokenAddress) external {
    (int64 responses, int32 tokenType) = HederaTokenService.getTokenType(tokenAddress);
    int associateResponse = HederaTokenService.associateToken(sender, tokenAddress);

    if (responses == HederaResponseCodes.SUCCESS && tokenType == 0 && associateResponse == HederaResponseCodes.SUCCESS) {
      associateCampainer[sender][tokenAddress][FUNGIBLE] = true;
    } else if (responses == HederaResponseCodes.SUCCESS && tokenType == 1 && associateResponse == HederaResponseCodes.SUCCESS) {
  ...
  //============================================
  function transferHbar(address payable receiverAddress, address campaignAddress, uint256 amount) public {
   ...
   */
  function updateBalance(address campaigner, uint256 amount, bool deposit) public payable {
    require(_addCampaign.has(msg.sender), "User has no permission to update");
    if (deposit) {
   ...
   * @param amount amount to be updated
   */
  function addCampaign(address campaignAddress, address campaigner, uint256 amount) public {
    require(_addCampaign.has(msg.sender));
   ...
   */
  function expiryCampaign(address campaignAddress, address campaigner) public {
    require(_addCampaign.has(msg.sender));
  ...
  function closeCampaign(address campaignAddress, uint256 campaignExpiryTime) public {
    require(_addCampaign.has(msg.sender));
    require(isCampaignClosed[campaignAddress][HBAR] == false);
    isCampaignClosed[campaignAddress][HBAR] = true;
    campaignEndTime[campaignAddress][HBAR] = block.timestamp + campaignExpiryTime;
   ...
   */
  function getCampaignBalance(address campaignAddress) public view returns (uint256) {
   ...
   * @param amount amount to be updated
   */
  function payInteractorFromCampaignBalances(address campaignAddress, uint256 amount) public {
    require(_addCampaign.has(msg.sender));
   ...
   */
  function addFungibleAndNFTCampaign(address tokenId, address campaignAddress, address campaigner, int64 tokenAmount) public {
    require(_addCampaign.has(msg.sender));
    (, int32 tokenType) = HederaTokenService.getTokenType(tokenId);
    if (tokenType == 0) {
      require(tokenBalances[campaigner][tokenId][FUNGIBLE] >= uint64(int64(tokenAmount)));
      require(tokenCampaignBalances[campaignAddress][tokenId][FUNGIBLE] == 0);
      tokenBalances[campaigner][tokenId][FUNGIBLE] -= uint64(int64(tokenAmount));
      tokenCampaignBalances[campaignAddress][tokenId][FUNGIBLE] = uint64(int64(tokenAmount));
      emit NewCampaignIsAdded(campaignAddress, uint64(int64(tokenAmount)), FUNGIBLE);
    } else if (tokenType == 1) {
      int response = HederaTokenService.transferNFT(tokenId, campaigner, address(this), tokenAmount);
      if (response == HederaResponseCodes.SUCCESS) {
        tokenCampaignBalances[campaignAddress][tokenId][NFT] = uint64(int64(tokenAmount));
        emit NewCampaignIsAdded(campaignAddress, uint64(int64(tokenAmount)), NFT);
      } else {
        revert();
      }
    }
   ...
   * @param tokenAmount amount of token
   */
  function expiryFungibleAndNFTCampaign(address tokenId, address campaign, address campaigner, int64 tokenAmount) public {
    require(_addCampaign.has(msg.sender));
    (, int32 tokenType) = HederaTokenService.getTokenType(tokenId);
    if (tokenType == 0) {
      require(isCampaignClosed[campaign][FUNGIBLE] == true);
      require(block.timestamp > campaignEndTime[campaign][FUNGIBLE]);
      tokenCampaignBalances[campaign][tokenId][FUNGIBLE] -= uint64(int64(tokenAmount));
      tokenBalances[campaigner][tokenId][FUNGIBLE] += uint64(int64(tokenAmount));
      ...
      int response = HederaTokenService.transferNFT(tokenId, address(this), campaigner, tokenAmount);
      if (response == HederaResponseCodes.SUCCESS) {
        tokenCampaignBalances[campaign][tokenId][NFT] -= uint64(int64(tokenAmount));
  ...
  function closeFungibleAndNFTCampaign(address tokenId, address campaign, uint256 campaignExpiryTime) public {
    require(_addCampaign.has(msg.sender));
    (, int32 tokenType) = HederaTokenService.getTokenType(tokenId);
    if (tokenType == 0) {
      isCampaignClosed[campaign][FUNGIBLE] = true;
      campaignEndTime[campaign][FUNGIBLE] = block.timestamp + campaignExpiryTime;
      emit campaignClosed(campaign, FUNGIBLE);
    } else if (tokenType == 1) {
      isCampaignClosed[campaign][NFT] = true;
      campaignEndTime[campaign][NFT] = block.timestamp + campaignExpiryTime;
   ...
   * @param tokenId id of token
   */
  function addFungibleAndNFTCampaigner(address newCampaigner, address tokenId) public {
    require(_addCampaign.has(msg.sender));
    (int64 responses, int32 tokenType) = HederaTokenService.getTokenType(tokenId);
   ...
   * dangerous function, just for testing
   */
  function deleteFungibleAndNFTCampaigner(address campaigner, address tokenId) public {
    require(_addCampaign.has(msg.sender));
    (int64 responses, int32 tokenType) = HederaTokenService.getTokenType(tokenId);
    if (responses == HederaResponseCodes.SUCCESS && tokenType == 0) {
   ...
   * @param tokenId address of tokenId
   */
  function getFungibleAndNFTBalance(address campaigner, address tokenId) public returns (uint256 res) {
    (int64 responses, int32 tokenType) = HederaTokenService.getTokenType(tokenId);
    if (responses == HederaResponseCodes.SUCCESS && tokenType == 0) {
   ...
   * @param tokenId address of token
   */
  function getFungibleAndNFTCampaignBalance(address campaignAddress, address tokenId) public returns (uint256 res) {
    (int64 response, int32 tokenType) = HederaTokenService.getTokenType(tokenId);
   ...
   * @param tokenAmount amount of token
   */
  function distributeFungible(address tokenId, address campaigner, address campaign, int64 tokenAmount) external {
    require(_addCampaign.has(msg.sender));
    require(isCampaignClosed[campaign][FUNGIBLE] == true);
    if (associateCampainer[campaigner][tokenId][FUNGIBLE] == true) {
      int response = HederaTokenService.transferToken(tokenId, address(this), campaigner, tokenAmount);
      if (response == HederaResponseCodes.SUCCESS) {
        tokenCampaignBalances[campaign][tokenId][FUNGIBLE] -= uint64(int64(tokenAmount));
        tokenBalances[campaigner][tokenId][FUNGIBLE] += uint64(int64(tokenAmount));
      } else {
  ...
  function batchFungible(address tokenId, address campaign, address[] memory campaigner, int64 tokenAmount) public {
    require(_addCampaign.has(msg.sender));
    require(isCampaignClosed[campaign][FUNGIBLE] == true);
    for (uint256 i = 0; i < campaigner.length; i++) {
      int response = HederaTokenService.transferToken(tokenId, address(this), campaigner[i], tokenAmount);
      if (response == HederaResponseCodes.SUCCESS) {
        tokenCampaignBalances[campaign][tokenId][FUNGIBLE] -= uint64(int64(tokenAmount));
        tokenBalances[campaigner[i]][tokenId][FUNGIBLE] += uint64(int64(tokenAmount));
      }
    }
  }
   ...
   * @param tokenAmount amount of token
   */
  function emergencyWithdraw(address tokenId, address to, int64 tokenAmount) public {
    require(msg.sender == owner);
    int response = HederaTokenService.transferToken(tokenId, address(this), to, tokenAmount);
    if (response != HederaResponseCodes.SUCCESS) {
      revert();
    }
  }
  ...
  function distributeNFT(address campaign, address campaigner, address tokenId, int64 tokenAmount) public {
    require(_addCampaign.has(msg.sender));
    require(isCampaignClosed[campaign][NFT] == true);
    if (associateCampainer[campaigner][tokenId][NFT] == true) {
      ...
      if (response == HederaResponseCodes.SUCCESS) {
        tokenCampaignBalances[campaign][tokenId][NFT] -= uint64(int64(tokenAmount));
        tokenBalances[campaigner][tokenId][NFT] += uint64(int64(tokenAmount));
      } else {
        revert();
      }
    }
