...
const calculateRespectivewidth = (height: number): number =>
  Math.floor((height * 1200) / 310);
const calculateRespectivheight = (width: number): number =>
  Math.floor((width * 310) / 1200);

const HashbuzzLogoMainTransparent = ({
  className,
  width,
  height,
  colors,
    ...
    >
      ...
      <g>
        <g>
          ...
          <polygon
            fill={colors?.color2 ?? '#5265ff'}
            points='120.53 143.47 160.47 120.41 160.47 74.26 120.53 97.32 120.53 143.47'
          />
        </g>
        <g>
          <path
            fill={colors?.color2 ?? '#5265ff'}
            d='m240.2,101.3c0-2.38-1.27-4.58-3.33-5.77l-16.66-9.62-19.8-11.43v22.87s0,46.16,0,46.16l39.79-22.97v-19.24Z'
          />
          <path
            fill={colors?.color2 ?? '#5265ff'}
